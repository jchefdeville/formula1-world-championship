...
public class DriverScoreLoader {

	private static final Logger logger = LoggerFactory.getLogger(DriverScoreLoader.class);

	public static List<DriverScore> fromCsv(String filePath) {
		try (var reader = new CSVReader(new FileReader(filePath))) {
			return reader.readAll().stream()
					.skip(1) // Skip header row
					.map(record -> new DriverScore(
							parseInt(record[0]),
							parseInt(record[1]),
							parseInt(record[2]),
